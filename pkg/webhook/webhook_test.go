...
func TestHandleWebhook(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			jobName:               &jobName,
			expStatusCode:         500,
			expRespBody:           `failed on runner .*: no worker pool configured for label "custom-label"`,
			expectBuildCount:      0,
		...
		},
		{
			...
			jobName:              &jobName,
			expStatusCode:        500,
			expRespBody:          `failed on runner .*: no worker pool configured for label "old-label"`,
			expectBuildCount:     0,
			runnerLabelAliases: map[string]string{
				"old-label": "new-label",
			},
		...
		{
			name:                  "Workflow Job Queued - No Pool Found - Not Self-Hosted",
			payloadType:           payloadType,
			action:                queuedAction,
			runnerLabels:          []string{"non-existent-label"},
			payloadWebhookSecret:  serverGitHubWebhookSecret,
			contentType:           contentType,
			createdAt:             &queuedTime,
			startedAt:             nil,
			...
			jobName:               &jobName,
			expStatusCode:         500,
			expRespBody:           `no worker pool configured for label "non-existent-label"`,
			expectBuildCount:      0,
			supportedRunnerLabels: []string{"non-existent-label"},
		},
		{
		...
		},
		{
			...
			jobName:               &jobName,
			expStatusCode:         500,
			expRespBody:           `failed on runner .*: no worker pool configured for label "sh-ubuntu-latest"`,
			expectBuildCount:      0,
		...
		},
		{
			...
			jobName:               &jobName,
			expStatusCode:         500,
			expRespBody:           `failed on runner .*: no worker pool configured for label "ubuntu-20.04-n2d-standard-2"`,
			expectBuildCount:      0,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.action == "queued" && tc.expectBuildCount > 0 {
				...
				if diff := cmp.Diff(tc.expGCBBuildIDs, r.GCBBuildIDs); diff != "" {
					t.Errorf("GCBBuildIDs mismatch (-want +got): %s", diff)
				}
			} else {
				// For all other cases (e.g., "in_progress", "completed", or errors),
				// we still expect a plain text response.
				if tc.expRespBody != "" && !strings.Contains(resp.Body.String(), tc.expRespBody) {
					t.Errorf("expected response body to contain %q, but got %q", tc.expRespBody, resp.Body.String())
				}
			}
