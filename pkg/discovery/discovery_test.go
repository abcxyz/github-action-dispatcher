...
func TestRunnerDiscovery_Run(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			cloudbuildMock: &mockCloudBuildClient{},
			resourceManagerMock: &mockResourceManagerClient{
				searchProjectsErr: fmt.Errorf("failed to search projects"),
			},
			expErr: `failed to get projects: failed to search projects with query "parent:organizations/12345 labels.env=test": failed to search projects`,
		},
		{
